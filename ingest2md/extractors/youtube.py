"""YouTube → subtitle-first ingestion → ASR fallback → Chinese archive."""
import asyncio
import logging
import tempfile
...
class YouTubeExtractor:
    name = "YouTube 视频"
    description = "YouTube → 字幕优先，缺失时音频 ASR → 中文 Markdown"
    acquisition_plan = (
        "探测平台人工字幕",
        "没有人工字幕时探测自动字幕",
        "没有可用字幕时才下载音频并进入 ASR",
    ...
    def _extract(self, url: str, output_dir: Path) -> Document:
        ...
        with tempfile.TemporaryDirectory(prefix="ingest2md-youtube-") as temp:
            ...
            if track is not None:
                ...
                logger.info("未找到可用字幕；下载 YouTube 音频并进入 ASR fallback")
                _ffmpeg_bin("ffmpeg")
                _ffmpeg_bin("ffprobe")
