...
- 只做单视频 / 分享短链的轻量采集；
- Playwright 打开真实页面；
- 优先读取浏览器已经产生的详情响应，其次使用 DOM 暴露的直接媒体地址；
- DOM 只有 `blob:` 时，可最后回退浏览器已观察到的直接媒体网络响应；
- 网络响应不得提升到详情响应 / DOM 之前，且 `.m3u8` 不进入普通文件下载链路；
...
如果详情响应、DOM 直接媒体和浏览器网络媒体三类候选都不可用，或遇到登录墙，应明确失败，并建议用户下载后走本地媒体通道。
